...
// ── Load real e-books from ebooks.json (generated by scripts/fetch-books.js) ─
function loadFeaturedEbooks() {
  const ebooksPath = path.join(__dirname, 'ebooks.json');
  if (!fs.existsSync(ebooksPath)) {
    console.warn('⚠️  ebooks.json not found. Run: node scripts/fetch-books.js');
    return [];
  }
  try {
    const data = JSON.parse(fs.readFileSync(ebooksPath, 'utf-8'));
    console.log(`\n📚  Real e-books loaded from ebooks.json:`);
...
function loadBooksFromExcel() {
  try {
    ...
    if (!fs.existsSync(filePath)) {
      console.log('Excel file not found – skipping Excel books.');
      return [];
    }
...
// ── Books: list (lightweight — no pages in list to keep response small) ───────
...
app.post('/api/books', upload.single('bookFile'), (req, res) => {
  ...
  const bookPages = pages
    ? pages.split('\n\n').filter(p => p.trim())
    : [`"${title}" — digital preview coming soon.`];
...
app.listen(PORT, () => {
  ...
  console.log(`\n🚀  Smart Library backend → http://localhost:${PORT}`);
  console.log(`📖  Featured e-books : ${ebooks.length}`);
